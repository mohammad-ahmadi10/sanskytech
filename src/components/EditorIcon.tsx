...
const EditorIcon =  memo( ({ Icon, onIcon, shouldDisable, event , styles}: EditorIconType) => {
    ...
    return (
        ...
        >
                  <Icon  className={(shouldDisable && shouldDisable===true ) ? 'fill-black/30 dark:fill-white/30 text-sm sm:text-xl ' : 'fill-black/60 dark:fill-white cursor-pointer pointer-events-none text-sm sm:text-xl'}/>
          </div>
        )
});
